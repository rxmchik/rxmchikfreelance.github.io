//datepicker  phone_mask autosearch prostiezvonki google_charts
...
var dayArrayMed = new Array('Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб');
var dayArrayLong = new Array('Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота');
...
function displayDatePicker(dateFieldName, displayBelowThisObject, dtFormat, dtSep)
{
  ...
  drawDatePicker(targetDateField, x, y, dtFormat);
}
...
*/
function drawDatePicker(targetDateField, x, y, dtFormat)
{
  ...
  refreshDatePicker(targetDateField.name, dtFormat, dt.getFullYear(), dt.getMonth(), dt.getDate(), dt.getHours(), dt.getMinutes(), dt.getSeconds());
...
*/
function refreshDatePicker(dateFieldName, dtFormat, year, month, day, h, i, s)
{
  ...
  if ((month >= 0) && (year > 0)) {
	  ...
	  if(!i)  {i = 0;}
	  if(!s)  {s = 0;}
    thisDay = new Date(year, month, 1, h, i, s);
  } else {
    day = thisDay.getDate();
    thisDay.setDate(1);
  }
  ...
  if((month >= 0) && (year > 0)) {
	 dayToTime = new Date(year, month, day, h, i, s);
  } else {
	 dayToTime = new Date();
  }
  ...
  var crlf = "\r\n";
  var TR = "<tr class='dpTR' valign='middle'>";
  ...
  html += "<table border='0' width='100%' cellpadding='0' cellspacing='2' cols=7 class='dpTable'>" + crlf;
  html += TR_title;
  html += TD_buttons + getButtonCode(dateFieldName, thisDay, -1, "&lt;", dtFormat) + xTD;
  ...
  html += "<table border='0' cellpadding='0' cellspacing='0'><tr valign='top'><td>";
  html += "<table border='0' cellpadding='0' cellspacing='2' class='dpTable'>";
  html += TR_days;
  ...
  html += TR;
  //переделано  first, the leading blanks
  var pust = thisDay.getDay();
  if(pust == 0) {pust = 7;}
  if(pust == 1) {pust = 1;}
  if(pust == 2) {pust = 2;}
  if(pust == 3) {pust = 3;}
  ...
  //time
  if(dtFormat == "dmy h:i:s")  {
	  html += "<td width='7'></td>";
	  html += "<td><div style='width:70px; max-height:156px; overflow:auto; overflow-x:hidden;'><table border='0' cellpadding='0' cellspacing='2' class='dpTable'>";
	  tmptd = getDateString(dayToTime);
	  tmptd = tmptd.split(" ");

	  for(i=0;i<24;i++) {
		  if(i<10) {ih = "0"+i;} else {ih = i;}

		  html += "<tr class='dpTR' valign='middle' onclick=\"updateDateField('" + dateFieldName + "', '" + tmptd[0] + " "+ih+":00:00');\"><td class='dpTDTime'>"+ih+":00</td></tr>";
		  html += "<tr class='dpTR' valign='middle' onclick=\"updateDateField('" + dateFieldName + "', '" + tmptd[0] + " "+ih+":30:00');\"><td class='dpTDTime'>"+ih+":30</td></tr>";
	  }
	  html += "</table></div></td>";
  }
  ...
  html += "<tr valign='top'><td>";
  html += "<table width='100%' border='0' cellpadding='0' cellspacing='0'><tr valign='top' align='center'><td>";
  var today = new Date();
  var todayString = "Today is " + dayArrayMed[today.getDay()] + ", " + monthArrayMed[ today.getMonth()] + " " + today.getDate();
  html += TR_todaybutton + TD_todaybutton;
  html += "<span class='dpTodayButton' onclick=\"refreshDatePicker('" + dateFieldName + "', '"+dtFormat+"');\">Сегодня</span> ";
  html += "<span class='dpTodayButton' onclick='updateDateField(\"" + dateFieldName + "\");'>Закрыть</span>";
  html += xTD + xTR;
  html += "</table>";
  html += "</td></tr>";
...
a month.
*/
function getButtonCode(dateFieldName, dateVal, adjust, label, dtFormat)
{
  var newMonth = (dateVal.getMonth () + adjust) % 12;
  var newYear = dateVal.getFullYear() + parseInt((dateVal.getMonth() + adjust) / 12);

  var newH = dateVal.getHours();
  var newM = dateVal.getMinutes();
  var newS = dateVal.getSeconds();
  ...
  return "<span class='dpButton' onclick='refreshDatePicker(\"" + dateFieldName + "\", \""+dtFormat+"\", " + newYear + ", " + newMonth +", \"\", " + newH + ", " + newM + ", " + newS + ");'>" + label + "</span>";
...
function getDateString(dateVal)
{
  ...
  var hourString = dateVal.getHours();
  var minString = dateVal.getMinutes();
  var secString = dateVal.getSeconds();
  ...
  switch (dateFormat) {
	...
	case "dmy h:i:s" :
      return dayString + dateSeparator + monthString + dateSeparator + dateVal.getFullYear() +" "+ hourString +":"+ minString +":"+ secString;
    case "ymd" :
...
jQuery.autosearch = function(input,options) {
	// Create a link to self
	var me = this;
	// Create jQuery object for input element
	var $input = $(input).attr("phone", "off");
	...
	function selectItem(li,cl) {
		if(options.fieldFill) {
			if(!li) {
			...
			$input.val("");
			if(cl && $($input).attr('id') == 'billProductSearch') {
		...
		if(response) {
			...
			for(var i=0; i < num; i++) {
				if(response['text'][i]) {
					...
					if(response['func'] && response['func'][i]) {li.setAttribute("onclick",response['func'][i]);}
					ul.appendChild(li);
					$(li).hover(
						function() { $("li", ul).removeClass("ac_over"); $(this).addClass("ac_over"); active = $("li", ul).indexOf($(this).get(0)); },
						function() { $(this).removeClass("ac_over"); }
					).click(function(e) {e.preventDefault(); e.stopPropagation(); selectItem(this,1); if(options.refreshCache) {refreshCache(this.innerHTML);}});
				}
			}
			return ul;
		}
	...
	function loadFromCache(q) {
		...
		if (options.matchSubset) {
			for(var i = q.length - 1; i >= options.minChars; i--) {
				var qs = q.substr(0, i);
				var resp = eval("("+cache.data[qs]+")");
				if(resp && resp.text) {
					var res = []; res["text"] = []; res["func"] = [];
					for(var j = 0; j < resp.text.length; j++) {
						var x = resp.text[j];
						if(!matchSubset(x, q)) {
							delete(resp.text[j]);
							delete(resp.func[j]);
					...
					resp["text"] = resp["text"].filter(function(x) {
    					return x !== undefined && x !== null;
					});
					resp["func"] = resp["func"].filter(function(x) {
    					return x !== undefined && x !== null;
					});
	...
	function refreshCache(d) {
		for(var prop in cache.data) {
			var resp = eval("("+cache.data[prop]+")");
			if(resp && resp.text) {
				for(var j = 0; j < resp.text.length; j++) {
					var x = resp.text[j];
					if(x == d) {
						resp.text.splice(j, 1);
						resp.func.splice(j, 1);
					}
				}
				if(resp.text.length > 0) {
					cache.data[prop] = JSON.stringify(resp);
				} else {
					delete cache.data[prop];
					cache.length--;
				}
			}
		}
...
jQuery.fn.autosearch = function(url, options, data) {
	...
	options.fieldFill = options.fieldFill || false;
	options.selectFirst = options.selectFirst || false;
	options.selectOnly = options.selectOnly || false;
	options.refreshCache = options.refreshCache || false;
	...
	this.each(function() {
		var input = this;
		new jQuery.autosearch(input, options);
	});
...
jQuery.fn.autosearchArray = function(data, options) {
	return this.autosearch(null, options, data);
}
